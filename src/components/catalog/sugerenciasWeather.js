...
define({
    ...
    clima: {
        ...
        connect: (host) => {
            fetch(
                'https://api.open-meteo.com/v1/forecast?latitude=-0.2298&longitude=-78.5249&current=temperature_2m,weather_code,is_day&timezone=America%2FGuayaquil',
            )
                .then((res) => res.json())
                .then((data) => {
    ...
    render: (host) => {
        ...
        if (productos && productos.length > 0) {
            if (clima && (clima.type === 'rain' || clima.temp < 15)) {
                sugerencias = productos.filter(
                    (p) =>
                        p.nombre.toLowerCase().includes('cappuccino') ||
                        p.nombre.toLowerCase().includes('croissant') ||
                        p.nombre.toLowerCase().includes('flat'),
                );
            } else {
                sugerencias = productos.filter(
                    (p) =>
                        p.nombre.toLowerCase().includes('v60') ||
                        p.nombre.toLowerCase().includes('aguacate'),
                );
            }
        ...
        const handleAñadir = (p) => {
            host.dispatchEvent(
                new CustomEvent('agregar', { detail: p, bubbles: true, composed: true }),
            );
        };
                ...
                ${clima
                    ? html`
                          <div class="weather-widget">
                              <div class="weather-icon">${clima.icon}</div>
                                      ...
                                      <div class="sug-content">
                                          <div class="sug-header-row">
                                              ...
                                              <span class="sug-price"
                                                  >$${parseFloat(p.precio).toFixed(2)}</span
                                              >
                                          </div>
