...
define({
            ...
            fetchWithTimeout('http://localhost:3000/api/productos/categorias')
                ...
                .then((cats) => {
                    ...
                    return Promise.all(
                        cats.map((cat) =>
                            fetchWithTimeout(
                                `http://localhost:3000/api/productos/categoria/${cat.slug}`,
                            )
                                .then((r) => r.json())
                                .then((prods) =>
                                    Array.isArray(prods)
    ...
    render: (host) => {
        const { categorias, productosFiltrados, categoriaActiva, cartItems, cargando, error } =
            host;
                    ...
                    ${error
                        ...
                        : cargando
                          ...
                          : productosFiltrados.length === 0
                            ? html`<div class="empty">No hay productos en esta categoría.</div>`
                            : html`
                        ...
                        <div class="cart-header">
                            ...
                            ${cartItems.length > 0
                                ? html`<span class="item-count"
                                      >${cartItems.reduce((s, i) => s + i.cantidad, 0)} items</span
                                  >`
                                : html``}
                        </div>
