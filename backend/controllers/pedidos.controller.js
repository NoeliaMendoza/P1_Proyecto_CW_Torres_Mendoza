  ...
  try {
    ...
    for (const item of items) {
      await db.query(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `, [pedidoId, item.producto_id, item.nombre, item.opcion_grupo, item.opcion_valor, item.precio_unit, item.cantidad]);
    }
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [itemRows] = await db.query('SELECT * FROM pedido_items WHERE pedido_id = ?', [req.params.id]);
...
router.put('/:id', async (req, res) => {
  ...
  try {
    await db.query('UPDATE pedidos SET estado = ?, atendido_por = ? WHERE id = ?', [estado, atendido_por, req.params.id]);
